...
<body class="bg-gray-50 min-h-screen">
  <div class="max-w-4xl mx-auto py-8 px-4">
    ...
    <div class="mt-8 bg-white shadow rounded-lg p-6">
      ...
      <form method="post" action="/cpsproject/admin/pathways/<?php echo $pathway['id']; ?>/edit" enctype="multipart/form-data" class="space-y-6">
        <?php echo \core\CSRF::inputField(); ?>

        <div>
          <label for="name" class="block text-sm font-medium text-gray-700">Pathway Name</label>
          <input type="text" id="name" name="name" required value="<?php echo htmlentities($pathway['name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
        ...
        <div>
          ...
          <?php if (!empty($pathway['image_url'])): ?>
            <img src="<?php echo htmlentities($pathway['image_url'], ENT_QUOTES, 'UTF-8'); ?>" alt="Current Image" class="h-20 w-auto my-2 rounded">
          <?php endif; ?>
        ...
        <div class="pt-4">
          <button type="submit" class="bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700">Update Pathway</button>
        </div>
      </form>
    </div>
  </div>
